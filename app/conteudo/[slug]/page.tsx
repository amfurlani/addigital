...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const a=articles.find(x=>x.slug===slug);if(!a)notFound();const author=lawyers.find(x=>x.slug===a.authorSlug);return <article><header className="article-hero"><div className="container narrow"><span className="eyebrow">{a.category}</span><h1>{a.title}</h1><p>{a.excerpt}</p><div className="article-meta"><span>{a.date}</span><span>{a.readTime} de leitura</span>{author&&<Link href={`/equipe/${author.slug}`}>{author.name}</Link>}</div></div></header>{a.podcast?.embedUrl && (
  <div className="container">
    <section className="article-podcast">
      <div className="article-podcast-header">
        <span className="podcast-eyebrow">OUÇA TAMBÉM</span>
...
<div className="container article-body">{a.body.map((p,i)=><p key={i}>{p}</p>)}</div>{a.podcast && (
      <div className="container">
